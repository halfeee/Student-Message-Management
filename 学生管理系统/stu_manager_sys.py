...
def to_change_manager_inf():
    return render_template('manager_chg_information.html',account=session['account'],name=session['name'],sex=session['sex']);
...
def to_stu_update():
    ...
    result = result[0];
    print(result);
    return render_template('stu_update.html',result = result);
